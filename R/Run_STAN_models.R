...
# run the first, simple model (this should take about 10 mintues)
model.1 <- stan(file = "tutorial/modelcode/model_treesize_only.stan", # path to the .STAN file, which contains the actual model code
                data = model.data, # the data to use in the model
                iter = 3000, # the number of iterations to run the model for
...
saveRDS(model.1, "tutorial/outputs/model.1.RDS")
...
# ------------------------------------------------------------------------------
# Run model 2: TREE size + a time-varying climate variable (Maximum temperature)
# ------------------------------------------------------------------------------
...
model.2 <- stan(file = "tutorial/modelcode/model_treesize_Tmax.stan",
                data = model.data,
                iter = 3000, chains = 2, verbose=FALSE, control =  list(max_treedepth = 15),
                #sample_file = model.name,
                #adapt_delta = 0.99,
                pars = c("mu", "sigma_inc", "sigma_add", "sigma_dbh","betaTmax", "betaX","alpha_TREE","x", "inc"))

# just as with model 1, lets look at the diagnostics:
# get model diagnostics and save these to look at
...
par.names = c("mu", "sigma_inc", "sigma_add", "sigma_dbh", "betaX","betaTmax", "alpha_TREE[1]")
...
# or they could cause problems with sampling/identifiablility
png(height = 7, width = 7, units = "in", res = 100, paste0("tutorial/outputs/pairs_plot_tutorial_", model.name, ".png"))
pairs(model.2, pars = c("mu", "sigma_inc", "sigma_add", "sigma_dbh", "betaX","betaTmax", "alpha_TREE[1]", "alpha_TREE[2]"))
dev.off()
# sigma_inc and sigma_add have somme posterior correlations, as do the alpha_TREE values with the tree size effect
...
# --------------------------------------------------------------------------------------------------------
# Run model 3: TREE size + two time-varying climate variables (Maximum temperature + total precipitation)
# --------------------------------------------------------------------------------------------------------
# Second, run a model that has tree size and a single climate variable in in it
# this model takes a little longer to run, so we will just run it with 2 chains to get some results
model.name <- "TmaxPPTX"
# add in the tree size effect with out the year random effects but with climate
model.3 <- stan(file = "tutorial/modelcode/model_noyearRE_X.stan",
                data = model.data,
                iter = 3000, chains = 2, verbose=FALSE, control =  list(max_treedepth = 15),
                #sample_file = model.name,
                #adapt_delta = 0.99,
...
#--------------------------------------------------------------------------------------
# Run a fourth model, but this time add in a plot level competition variable SDI
#--------------------------------------------------------------------------------------
...
# add in the tree size effect with out the year random effects but with climate
model.4 <- stan(file = "tutorial/modelcode/model_treesize_climate_sdi.stan",
                data = model.data,
                iter = 3000, chains = 2, verbose=FALSE, control =  list(max_treedepth = 15),
                #sample_file = model.name,
                #adapt_delta = 0.99,
...
# get model diagnostics and save these to look at
sampler_params <- get_sampler_params(model.4, inc_warmup = FALSE)
...
# get the convergence statistics of the model:
fit_ssm_df <- as.data.frame(model.4) # takes awhile to convert to df
Rhats <- apply(fit_ssm_df, 2, Rhat)
hist(Rhats)
# most of the R hat values are below 1.01
ESS_bulks <- apply(fit_ssm_df, 2, ess_bulk)
...
# or they could cause problems with sampling/identifiablility
png(height = 7, width = 7, units = "in", res = 100, paste0("tutorial/outputs/pairs_plot_tutorial_", model.name, ".png"))
pairs(model.4, pars = c("mu", "sigma_inc", "sigma_add", "sigma_dbh", "betaX","betaPrecip", "alpha_TREE[1]", "alpha_TREE[2]"))
dev.off()
# sigma_inc and sigma_add have somme posterior correlations, as do the alpha_TREE values with the tree size effect
...
# seelect the diameter estimates (x.pred) from the model.3 dataframe
x.pred <- dplyr::select(as.data.frame(model.4),"x[1,1]":paste0("x[", model.data$Nrow, ",", model.data$Ncol,"]"))
# select the increment estimates (inc) from the model.3 dataframe
inc.pred <- dplyr::select(as.data.frame(model.4),"inc[1,1]":paste0("inc[", model.data$Nrow, ",", model.data$Ncol,"]"))
